                ...
                <div class="card my-4">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('dashboard.usersTrain.update', $usersTrain) }}">
                                    ...
                                    <div class="mb-3 col-md-6">
                                        <select id="user_id"
                                            ...
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}"
                                                    @if ($usersTrain->user_id == $user->id) selected @endif>
                                                    {{ $user->name }}</option>
                                            @endforeach
                                        </select>
                                            ...
                                            @foreach ($trains as $train)
                                                @if ($train->category->user_id == auth()->id())
                                                    <option value="{{ $train->id }}"
                                                        @if ($train->id == $usersTrain->train_id) selected @endif>
                                                        {{ $train->title }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="mb-3 col-md-6">
                                    <input id="date" type="date"
                                        ...
                                        name="date" value="{{ old('date', $usersTrain->date) }}" required>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="mb-3 col-md-6">
                                    <input id="time" type="time"
                                        ...
                                        name="time" value="{{ old('time', $usersTrain->time) }}" required>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="mb-3 col-md-6">
                                    <input id="duration" type="number"
                                        ...
                                        name="duration" value="{{ old('duration', $usersTrain->duration) }}" required>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="mb-3 col-md-6">
                                    <textarea id="notes" class="form-control border border-2 p-2 @error('notes') is-invalid @enderror" name="notes"
                                        required>{{ old('notes', $usersTrain->notes) }}</textarea>
                            ...
                            @if (!auth()->user()->hasRole('trainee'))
                                <div class="form-group row  gx-4 mb-2">
                                    <label for="user_id" class="col-md-4 col-form-label text-md-right">Status</label>

                                    <div class="mb-3 col-md-6">
                                        <select id="status"
                                            class="form-control border border-2 p-2 @error('status') is-invalid @enderror"
                                            name="status" required>
                                            <option value="pending" @if ($usersTrain->status == 'pending') selected @endif>
                                                Pending</option>
                                            <option value="accepted"
                                                @if ($usersTrain->status == 'accepted') selected @endif>Accepted</option>
                                            <option value="reject" @if ($usersTrain->status == 'reject') selected @endif>
                                                Reject</option>
                                            <option value="done" @if ($usersTrain->status == 'done') selected @endif>
                                                Done</option>
                                        </select>
